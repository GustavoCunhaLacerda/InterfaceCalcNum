...
from methods.integracao.simpson import *
...
class PaginaIntegracaoSimp(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)

        title_label = tk.Label(self,
            text = "ITEGRAÇÃO SIMPSON",
            font = FONTE_TITULO,
            foreground = "white",
            bg = BG_COLOR,
            heigh = 4
        ...
        calcular_button = tk.Button(
            ...
            bg = "white",
            width = BUTTON_WIDTH,
            command = self.integrar_por_simpson
        )
    ...
    def integrar_por_simpson(self):
        ...
        inicio = time.time()
        area = regra_simpson(f.f, a, b, pontos)
        fim = time.time()
        ...
        # plot do grafico #
        if self.graph.get() == 1:
            grafico_simpson(f.f, int(a), int(b), pontos, self.func_entry.get().replace("**", "^").replace("*", ""))
            plt.show()
